...
class OCWBError(Exception):
    """Generic base class for OCWB exceptions"""
    pass


class ConfigurationError(OCWBError):
...
class APIRequestError(OCWBError):
    """
    Error class that represents network/infrastructural failures when invoking OCWB Weather API, in
...
class APIResponseError(OCWBError):
    """
    Generic base class for exceptions representing HTTP error status codes in OCWB Weather API
    responses
    """
    pass
