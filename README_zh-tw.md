...
### v1.3.42
- 修正同時存在多項警特報時，各警報誤共用第一筆發布時間、有效時間與官方說明的問題；現在會逐一保留每個 CWA dataset 的原始資料。
- 陸上強風特報會顯示官方黃／橙／紅燈號、平均風與陣風門檻、換算風速下限、有效時間、危險程度，以及作物、棚架、網布與溫室的具體風險及防護建議。
- 農業影響明確標示為依 CWA 風力門檻整理的提示，不會冒充 CWA 官方農損預測。

### v1.3.41
- 熱帶氣旋只有在「預測接近臺灣、可能影響設定位置、CWA 官方颱風警報有效」三項條件同時成立時才進入告警；其餘活動中系統改為 `monitoring`，不建議推播。
- 新增距臺灣、距設定位置、最近接近時間與各判斷條件 attributes，讓 dashboard 與 automation 可以說明為何告警或僅監測。
- 尚未命名的熱帶性低氣壓改用 CWA TD 編號顯示，例如 `未命名熱帶性低氣壓 TD12`，不再顯示 `未知（）`。
...
| `sensor.opencwa_an_ping_qu_typhoon_warning_notification` | 可直接給 `notify.*` 使用的官方颱風警報通知內容；attributes 內含 `title`、`message`、`severity`、`summary`。 |
| `sensor.opencwa_an_ping_qu_tropical_cyclone` | 目前活動中熱帶氣旋數量；屬性包含最新定位、分析路徑、預測路徑、風速、氣壓、暴風半徑等。 |
| `sensor.opencwa_an_ping_qu_tropical_cyclone_notification` | 熱帶氣旋風險通知；未同時符合接近臺灣、影響設定位置及官方警報時為 `monitoring`，符合後由官方警報通知作為主要告警。 |
...
| `sensor.opencwa_an_ping_qu_weather_alert_notification` | 可直接給 `notify.*` 使用的重大氣象警特報通知內容；會整理命中區域、比對方式、特殊區域與 CWA 說明文字。陸上強風特報另提供官方燈號、風級／風速門檻，以及明確標示來源的農作物與溫室風險提示。 |
...
#### 通知 sensor attributes

出現於：

- `sensor.*_typhoon_warning_notification`
- `sensor.*_tropical_cyclone_notification`
- `sensor.*_weather_alert_notification`

這三個 sensor 是為 Home Assistant automation 準備的「可直接使用通知訊息」。使用者不需要在 automation 裡自行組合大量 template，只要讀取 `title` 和 `message` 即可送到 `notify.*`。

| Attribute | 型別 / 範例 | 意義 | 用途 |
| --- | --- | --- | --- |
| `active` | boolean | 這則通知目前是否應視為有效告警。 | automation 可用來決定是否發送。 |
| `status` | string：`active` / `inactive` / `monitoring` / `suppressed` | `monitoring` 代表有熱帶氣旋或官方警報資料，但尚未同時符合三項告警條件；`suppressed` 代表已由更高優先級通知涵蓋。 | automation 建議只在 state 變成 `active` 時推播。 |
| `severity` | string：`critical` / `warning` / `advisory` / `info` | 建議通知嚴重程度。官方颱風警報通常為 `critical`，重大氣象警特報為 `warning`，熱帶氣旋預先注意為 `advisory`。 | 可對應不同手機推播 priority、聲音或 dashboard 顏色。 |
| `title` | string | 已整理好的通知標題，例如 `⚠️ CWA 官方颱風警報`。 | 直接填入 `notify.*` 的 `title`。 |
| `message` | string | 已整理好的完整通知內文，包含颱風名稱、警報類型、命中區域、影響區域、最新定位等。 | 直接填入 `notify.*` 的 `message`，降低 automation template 複雜度。 |
| `summary` | string | 短摘要，例如颱風警報 headline、警特報種類或熱帶氣旋名稱。 | 適合放在 dashboard card、通知列表或較短的訊息格式。 |
| `source_dataset` | string，例如 `W-C0034-001` | 這則通知主要來自哪個 CWA 資料集。 | debug 或追蹤資料來源。 |
| `forecast_approaches_taiwan` | boolean | CWA 預測路徑是否進入距臺灣 800 公里的監測範圍；目前定位不會被當成預測。 | 說明是否具備接近臺灣條件。 |
| `location_at_risk` | boolean | 路徑是否進入距設定位置 500 公里的潛在影響範圍。 | 說明是否具備設定位置影響條件。 |
| `official_warning_active` | boolean | CWA 官方颱風警報是否有效。 | 告警必要條件，不由風速自行推算。 |
| `closest_distance_to_taiwan_km` / `closest_distance_to_location_km` | number 或 `null` | CWA 預測路徑的最近距離。 | dashboard 顯示判斷依據。 |
| `closest_approach_time` | datetime string 或 `null` | 距設定位置最近的路徑點時間。 | 顯示可能最接近時間；CWA 未提供時間時可為 `null`。 |
...
action:
  - service: notify.mobile_app_your_phone
    data:
      title: "{{ state_attr(trigger.entity_id, 'title') }}"
