...
| `sensor.opencwa_an_ping_qu_tropical_cyclone_notification` | 可直接給 `notify.*` 使用的熱帶氣旋預先注意內容；若已有官方颱風警報，state 會是 `suppressed`，避免重複告警。 |
...
| `status` | string：`active` / `inactive` / `suppressed` | 通知狀態。`active` 代表應通知；`inactive` 代表目前無需通知；`suppressed` 代表有資料但被更高優先級告警覆蓋，例如已有官方颱風警報時抑制熱帶氣旋預先注意。 | 可直接當 sensor state 使用，也可避免重複通知。 |
